...
def run_golden_desktop_robot_arm(
    workspace: str | Path,
    *,
    mode: str = "contract",
    ...
    backend = backend or WorkflowConsoleBackend(project_root=project, workspace_root=workspace_path)
    emit("workspace", "running", "Initializing workspace and Work")
    backend.create_workspace(name="CadFlow Golden Examples", advancement_mode="manual_confirm")
    work_id = _next_work_id(backend, WORK_ID)
    run_id = f"{work_id}_root"
    backend.create_work(
    ...
    emit("workspace", "completed", "Created Golden Desktop Robot Arm Work")

    emit("requirement", "running", "Interpreting the desktop robot arm request")
    backend.run_stage_by_id(run_id, "requirement", root=runs_root)
    emit("requirement", "completed", "Requirement questions created")
    emit("clarification", "running", "Applying fixed example inputs")
    ...
    emit("clarification", "completed", "Clarification applied to requirement_v2")
    emit("planning", "running", "Creating assembly plan")
    backend.run_stage_by_id(run_id, "planning", root=runs_root)
    emit("planning", "completed", "Planning completed")
    emit("assembly_plan", "completed", "Identified candidates, references, and selected upper_link")

    actions = WorkflowConsoleActions(backend)
    emit("part_request", "running", "Creating selected upper_link part request")
    actions.create_part_request(run_id, part_id="upper_link", root=runs_root)
    emit("part_request", "completed", "Part request ready for review")
    emit("part_review", "running", "Reviewing selected part request")
    actions.review_part_request(run_id, root=runs_root)
    emit("part_review", "completed", "Part request approved")
    emit("handoff", "running", "Creating reviewed part handoff")
    actions.create_reviewed_handoff(run_id, root=runs_root)
    emit("handoff", "completed", "Reviewed handoff ready")
    emit("cad_ir_draft", "running", "Normalizing upper_link to a generic CAD IR family")
    if mode == "full":
        emit("cadquery_generation", "running", "Generating STEP and STL with CadQuery")
    actions.create_reviewed_part(run_id, root=runs_root, execute_cad=mode == "full")
    emit("cad_ir_draft", "completed", "Generic link-like CAD IR created")
    emit("input_ir_validation", "completed", "CAD IR validated and input_ir created")
    if mode == "full":
        emit("cadquery_generation", "completed", "STEP and STL generated")
        emit("part_result_review", "running", "Reviewing generated selected-part artifacts")
        actions.review_part_result(run_id, root=runs_root)
        emit("part_result_review", "completed", "Part result review completed")
    else:
        emit("cadquery_generation", "skipped", "CAD execution intentionally skipped in contract mode")
        emit("part_result_review", "skipped", "Part result review is not expected without CAD execution")
    emit("workflow_review", "running", "Creating workflow review")
    actions.create_workflow_review(run_id, root=runs_root)
    emit("workflow_review", "completed", "Workflow review completed")

    run_path = backend.resolve_run(run_id, root=runs_root)
    emit("comparison", "running", "Comparing actual artifacts with the golden specification")
    comparison = compare_golden_actual_to_expected(run_path, mode=mode)
    _write_comparison(run_path, comparison)
    emit("comparison", "completed" if comparison["passed"] else "failed", "Golden comparison passed" if comparison["passed"] else "Golden comparison found mismatches")
    execution = {
        "status": "full_complete" if mode == "full" else "contract_complete",
        "execution_skipped": mode == "contract",
        "cad_ir_validated": True,
        "input_ir_created": True,
        "step_stl_expected": mode == "full",
        "assembly_generated": False,
        "result_scope": "single_generic_concept_part",
    }
    (run_path / "golden_example.json").write_text(
        json.dumps({"schema_version": 1, "mode": mode, "work_id": work_id, "run_id": run_id, "execution": execution, "progress": progress, "comparison": comparison}, indent=2, sort_keys=True) + "\n",
        encoding="utf-8",
    )
...
def compare_golden_actual_to_expected(run_path: str | Path, *, mode: str) -> dict[str, Any]:
...
def _next_work_id(backend: WorkflowConsoleBackend, base: str) -> str:
    ...
    for attempt in range(2, 10_000):
        candidate = f"{base}_attempt_{attempt}"
        if candidate not in existing:
            return candidate
    raise FileExistsError("golden example attempt id space is exhausted")


# Backward-compatible names retained for the original executable fixture API.
run_golden_workflow = run_golden_desktop_robot_arm
compare_actual_to_expected = compare_golden_actual_to_expected
...
def main() -> None:
    ...
    result = run_golden_desktop_robot_arm(args.workspace, mode=args.mode)
