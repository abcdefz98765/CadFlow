...
- Provide path-safe run-id operations for future HTTP routes, resolving only under configured `outputs/` and `runs/` roots while rejecting absolute paths, traversal, and path separators.
