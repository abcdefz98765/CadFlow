...
### A-004: No preview image generation in CadQuery pipeline
**Severity:** Medium
**Status:** Open (evaluated in `docs/web_preview.md`, planned for Phase 4/5)

The PRD requires preview capability. Currently no preview image or viewer is generated by the CadQuery side. Evaluation complete: recommended path is a lightweight three.js STL viewer (`web-viewer/index.html`) with optional trimesh-based glTF/GLB export. See `docs/web_preview.md` for full evaluation. FreeCAD screenshots remain GUI-only. Blender rendering is deferred to Phase 5.

**Recommendation:** Implement `web-viewer/index.html` prototype (Phase 4), then optionally add GLB export to the pipeline (Phase 5).
