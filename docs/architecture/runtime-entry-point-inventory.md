...
| `run_work_part_design_episode` / `POST /api/works/{work_id}/parts/{part_job_id}/design-episodes` | Backend/API | target product | Routes an owned attempt through `WorkOrchestrator` and `AgentDesignPort` for provider-selected contract validation or attested model-program execution evidence; no reviewable publication, lineage change, or acceptance authority. |
