...
| `run_work_part_design_episode` / `POST /api/works/{work_id}/parts/{part_job_id}/design-episodes` | Backend/API | target product | Routes an owned attempt through `WorkOrchestrator` and `AgentDesignPort` for provider-selected contract validation or attested model-program execution, then permits only the CadFlow-owned gate to publish fully cross-checked reviewable evidence; no lineage or acceptance mutation. |
| `accept_work_reviewable_result` / `POST /api/works/{work_id}/parts/{part_job_id}/reviewable-results/{reviewable_result_id}/accept` | Backend/API | target product | Explicit user authority that accepts only the exact registered reviewable record/STEP pair and changes that Part Job's accepted-result pointer. |
| `revise_work_reviewable_result` / `POST /api/works/{work_id}/parts/{part_job_id}/reviewable-results/{reviewable_result_id}/revisions` | Backend/API | target product | Creates a new Part Job attempt Run from a registered reviewable result while preserving historical evidence and any prior accepted pointer. |
...
| `run_design_part_episode` | provider-selected contract/model-program episode preview | evaluation service | Called through the target-product `AgentDesignPort` for validation or attested execution evidence and remains directly usable for evaluation; no publication or acceptance authority. |
